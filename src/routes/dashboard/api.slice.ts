...
import { AddBudgetTableFormData, EditBudgetTableFormData, Table } from '@/routes/dashboard/budget/budget.types';
      ...
      queryFn: async (dashboardId: number) => {
        try {
          const { data, error } = await supabase
            .from('budget')
            .select(
              `id, name, amount, created_at, budget_categories (budget_id, category_id, amount, spent, categories (id, name))`,
            )
            .eq('dashboard_id', dashboardId);
