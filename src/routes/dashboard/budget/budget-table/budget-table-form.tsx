...
export const AddBudgetTableForm = ({ tables, handleModalClose }: AddBudgetTableFormProps) => {
  ...
  const onSubmit: SubmitHandler<AddBudgetTableFormData> = async () => {
    try {
  ...
  /**
   * When categories are loaded (after mount or update),
   * reset the form's categories field to match the latest categories.
   * This ensures the form reflects the actual categories from the API.
   */
  useEffect(() => {
...
export const EditBudgetTableForm = ({ table, handleModalClose }: EditBudgetTableFormProps) => {
  ...
  const onSubmit: SubmitHandler<EditBudgetTableFormData> = async () => {
    ...
    const isSameName = table.name === getValues('name');
    const isSameAmount = table.amount === getValues('amount');
